...
MicroPython module: https://docs.micropython.org/en/vab847696a/library/pyb.html
...
class CAN():
    ...
    def __init__(self, bus,  mode, baudrate=328125, *, prescaler=-1, polarity=1, phase=0, bits=8, firstbit=MSB, ti=False, crc=None) -> None:
        ...
    def init(self, mode, prescaler=100, *, sjw=1, bs1=6, bs2=8, auto_restart=False, baudrate=0, sample_point=75,
        num_filter_banks=14, brs_sjw=1, brs_bs1=8, brs_bs2=3, brs_baudrate=0, brs_sample_point=75) -> None:
        """
...
def millis() -> int:
    """
       ...
       请注意，如果发出 :meth:`pyb.stop()` 命令，则支持此函数的硬件计数器将在“休眠”状态的持续时间内暂停。这会影响 :meth:`pyb.elapsed_millis()` 的结果。
    """
    ...
def micros() -> int:
    """
       ...
       请注意，如果发出 :meth:`pyb.stop()` 命令，则支持此函数的硬件计数器将在“休眠”状态的持续时间内暂停。这会影响 :meth:`pyb.elapsed_micros()` 的结果。
    """
    ...
def elapsed_millis(start) -> int:
    """
       ...
       此函数会处理计数器溢出，并始终返回正数。这意味着它可用于测量长达约 12.4 天的时间段。
...
def elapsed_micros(start) -> int:
    """
       ...
       此函数会处理计数器溢出，并始终返回正数。这意味着它可用于测量长达约 17.8 分钟的时间段。
...
def hard_reset() -> NoReturn:
    """
       以类似于按下外部 RESET 按钮的方式重置 OpenMV Cam。
    """
    ...
def bootloader() -> None:
    """
...
def fault_debug(value) -> None:
    """
       启用或禁用硬件故障调试。当底层系统出现致命错误，如无效内存访问时，会发生硬件故障。
    
       如果 *value* 参数为 ``False``，则如果出现硬件故障，则板子将自动复位。
    
       如果 *value* 为 ``True``，则当板子发生硬件故障时，它将打印寄存器和堆栈跟踪，然后无限循环 LED。
...
def disable_irq() -> Incomplete:
    """
       禁用中断请求。返回先前的IRQ状态：对于禁用的/启用的IRQ，分别返回 ``False``/``True``。此返回值可传递给enable_irq以恢复IRQ到其原始状态。
    """
    ...
def enable_irq(state=True) -> None:
    """
       启用中断请求。如果 ``state`` 为 ``True`` （默认值），则启用IRQ。如果 ``state`` 为 ``False``，则禁用IRQ。这个函数的最常见用法是将 ``disable_irq`` 返回的值传递给它，以退出临界区。
    """
    ...
def wfi() -> None:
    """
       ...
       这会执行一个 ``wfi`` 指令，该指令会降低MCU的功耗，直到发生任何中断（无论是内部还是外部），此时执行会继续。请注意，系统滴答中断每毫秒发生一次（1000Hz），因此此函数最多会阻塞1ms。
    """
    ...
def stop() -> Incomplete:
    """
       ...
       这将使功耗降低到低于500微安。要从此睡眠状态唤醒，需要外部中断或一个实时时钟事件。唤醒后，执行会在之前离开的地方继续。
    
       有关配置实时时钟唤醒事件，请参阅 :meth:`rtc.wakeup`。
    """
    ...
def standby() -> Incomplete:
    """
       将 OpenMV Cam 置于“深度睡眠”状态。
    
       这将使功耗降低到低于 50 微安。要从此睡眠状态唤醒，需要一个实时时钟事件。唤醒后，系统会执行硬复位。
...
def have_cdc() -> bool:
    """
       ...
       ``Note:`` 此函数已被弃用。请改用 pyb.USB_VCP().isconnected() instead。
    """
    ...
def hid(hidtuple:Tuple) -> Incomplete:
    """
       ...
       ``Note:`` 此函数已被弃用。请改用 :meth:`pyb.USB_HID.send()`。
    """
    ...
...
def mount(device, mountpoint, *, readonly=False, mkfs=False) -> int:
    """
       ``Note:`` 此函数已被弃用。设备的挂载和卸载应通过 :meth:`vfs.mount` 和 :meth:`vfs.umount` 来执行。
    
       挂载一个块设备并将其作为文件系统的一部分使其可用。``device`` 必须是一个提供块协议的对象。（以下内容也已弃用。请参见 :class:`vfs.AbstractBlockDev` 了解正确的块设备创建方式。）
       ...
       ``readblocks`` 和 ``writeblocks`` 应该在设备上从块号 ``blocknum`` 开始将数据复制到 ``buf`` 和设备之间。``buf`` 将是一个长度为 512 的倍数的字节数组。如果未定义 ``writeblocks``，则将只读方式挂载设备。这两个函数的返回值将被忽略。
    
       ``count`` 应该返回设备上可用的块数。``sync``，如果实现了，应该同步设备上的数据。
    
       参数 ``mountpoint`` 是文件系统根目录中挂载设备的位置。它必须以斜杠开头。
    
       如果 ``readonly`` 为 ``True``，则以只读方式挂载设备，否则以读写方式挂载。
    
       如果 ``mkfs`` 为 ``True``，则如果不存在文件系统，则创建一个新的文件系统。
    """
    ...
...
def usb_mode(modestr: Optional[Any]=None, port=-1, vid=0xf055, pid=-1, msc=(), hid=hid_mouse, high_speed=False) -> str:
    """
       ...
       *vid* 和 *pid* 参数允许您指定 VID（供应商 ID）和 PID（产品 ID）。 *pid* 值为 -1 将根据 *modestr* 的值选择 PID。
